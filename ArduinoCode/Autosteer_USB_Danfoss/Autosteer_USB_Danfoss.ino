  ...
  // PID variables
  float outputSum, lastSteerAngleError, dInput, pidOutput;
    ...
    */
    ...
    if (watchdogTimer < 10)
      { 
        ...
        if (aogSettings.CytronDriver) digitalWrite(PWM2_RPWM, 1);        
        else digitalWrite(DIR1_RL_ENABLE, 1);          
        
        steerAngleError = steerAngleActual - steerAngleSetPoint;   //calculate the steering error
        digitalWrite(PWM2_RPWM, 1); // turn the V+ valve pin on
    ...
    else
      {
        ...
        //Disable H Bridge for IBT2, hyd aux, etc for cytron
        if (aogSettings.CytronDriver) digitalWrite(PWM2_RPWM, 0);        
        else digitalWrite(DIR1_RL_ENABLE, 0);          

        outputSum = 0; // clear any PID residue values
        lastSteerAngleError = 0;
        digitalWrite(PWM2_RPWM, 0); // turn the V+ valve pin off for safety
      ...
      if (aogSettings.BNOInstalled)
        ...
        Serial.print(pulseCount);                 //No IMU installed
